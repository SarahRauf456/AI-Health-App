...
def show_analytics_ad():
    ...
    # --- CHART 1: Daily Total Calories (Bar Chart) ---
    st.subheader("📅 Daily Calorie Intake")
    # Group by Date and sum the Calories
    ...
    # --- CHART 2: Macros for a Specific Date (Pie Chart) ---
    st.subheader("🍰 Daily Macro Breakdown")

    selected_date = st.date_input("Select a Date to View Macros", datetime.now())

    # FIX 1: Create 2 columns properly
    col1, col2 = st.columns(2)

    with col1:
        st.markdown("#### **Actual Intake**")
        # Filter data for the selected date
        mask = df_log["Date"] == pd.Timestamp(selected_date)
        day_data = df_log[mask]

        if not day_data.empty:
            total_p = day_data["Protein"].sum()
            total_c = day_data["Carbs"].sum()
            total_f = day_data["Fats"].sum()
            ...
            if total_p + total_c + total_f > 0:
                ...
                st.warning(f"No macros recorded for {selected_date.strftime('%Y-%m-%d')}.")
        else:
            st.info(f"No meals logged on {selected_date.strftime('%Y-%m-%d')}.")

    with col2:
        st.markdown("#### **Target Goal**")
        # FIX 2: Correctly handle dictionary return from generate_nutrition_plan
        plan = generate_nutrition_plan()

        if plan:
            prot_g = plan["Protein (g)"]
            carbs_g = plan["Carbs (g)"]
            fats_g = plan["Fats (g)"]

            if prot_g + carbs_g + fats_g > 0:
                fig_pie1 = px.pie(
                    names=["Protein", "Carbs", "Fats"],
                    values=[prot_g, carbs_g, fats_g],
                    title="Recommended Goal",
                    color_discrete_sequence=px.colors.qualitative.Pastel,
                    hole=0.4
                )
                # FIX 3: Plot the correct figure (fig_pie1)
                st.plotly_chart(fig_pie1, use_container_width=True)
        else:
            st.warning("Please set up your profile to see targets.")
    ...
    # Resample data by Week ('W' means Weekly frequency)
    weekly_stats = df_log.set_index("Date").resample('W')["Calories"].sum().reset_index()

    # Create a clean label for the week
...
def get_simple_streak(df_food):
    # 1. Clean and convert to a Set of dates (Removes duplicates automatically)
    ...
    # 3. Determine start point (Start from Today if logged, else Yesterday)
